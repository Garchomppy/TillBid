...
export const ProfileView: React.FC = () => {
    ...
    const [idPhotos, setIdPhotos] = useState<{ front: string | null, back: string | null }>({ front: null, back: null });
                    ...
                    {isVerifying && !currentUser.isVerified ? (
                        <div className="glass-card p-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
                            <div className="flex items-center justify-between mb-10">
                                <div>
                                    <h3 className="text-3xl font-black text-text-main mb-2">Xác minh tài khoản</h3>
                                    <p className="text-text-muted font-bold text-sm">Vui lòng tải lên ảnh CCCD để nhận Tick xanh và tăng hạn mức giao dịch.</p>
                                </div>
                                <button 
                            ...
                            <div className="grid md:grid-cols-2 gap-8 mb-10">
                                ...
                                <div 
                                    ...
                                    className={`relative group cursor-pointer border-2 border-dashed rounded-[32px] p-8 transition-all h-60 flex flex-col items-center justify-center text-center
                                        ${idPhotos.front ? 'border-secondary bg-secondary/5' : 'border-border-main hover:border-primary/50 hover:bg-primary/5'}
                                    `}
                                >
                                    {idPhotos.front ? (
                                        <div className="flex flex-col items-center gap-4">
                                            ...
                                            <div className="font-black text-secondary">Mặt trước đã tải lên</div>
                                            <div className="text-[10px] font-bold text-text-muted uppercase">Nhấn để thay đổi</div>
                                        </div>
                                    ) : (
                                        <div className="flex flex-col items-center gap-4">
                                            <div className="w-16 h-16 bg-background rounded-full flex items-center justify-center text-text-muted group-hover:text-primary transition-colors">
                                                <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                                    <path strokeLinecap="round" strokeLinejoin="round" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" />
                                                    <path strokeLinecap="round" strokeLinejoin="round" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" />
                                                </svg>
                                            </div>
                                            <div className="font-black text-text-main">Mặt trước CCCD</div>
                                            <div className="text-[10px] font-bold text-text-muted uppercase">Nhấn để chọn ảnh</div>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div 
                                    ...
                                    className={`relative group cursor-pointer border-2 border-dashed rounded-[32px] p-8 transition-all h-60 flex flex-col items-center justify-center text-center
                                        ${idPhotos.back ? 'border-secondary bg-secondary/5' : 'border-border-main hover:border-primary/50 hover:bg-primary/5'}
                                    `}
                                >
                                    {idPhotos.back ? (
                                        <div className="flex flex-col items-center gap-4">
                                            ...
                                            <div className="font-black text-secondary">Mặt sau đã tải lên</div>
                                            <div className="text-[10px] font-bold text-text-muted uppercase">Nhấn để thay đổi</div>
                                        </div>
                                    ) : (
                                        <div className="flex flex-col items-center gap-4">
                                            ...
                                            <div className="font-black text-text-main">Mặt sau CCCD</div>
                                            <div className="text-[10px] font-bold text-text-muted uppercase">Nhấn để chọn ảnh</div>
                                        </div>
                                    )}
                                </div>
                            ...
                            <button 
                                ...
                                className={`w-full py-6 rounded-3xl font-black text-sm tracking-widest uppercase transition-all flex items-center justify-center gap-3
                                    ${!idPhotos.front || !idPhotos.back || isSubmitting 
                                        ? 'bg-background text-text-muted cursor-not-allowed border border-border-main' 
                                        : 'bg-primary text-white shadow-2xl shadow-primary/30 hover:shadow-primary/40 active:scale-95'}
                                `}
                            >
                                {isSubmitting ? (
                                    <>
