...
export const ProductDetailView: React.FC<ProductDetailProps> = ({ id }) => {
  ...
  const handleBid = () => {
    if (!currentUser) {
      store.addNotification("Vui lòng đăng nhập để đấu giá!", "warning");
      return;
    }
    ...
    if (
      ...
      product.sellerId === currentUser.name
    ) {
    ...
    store.placeBid(product.id, bidAmount);
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
        ...
        <div className="space-y-6">
          ...
          <div className="flex gap-4 overflow-x-auto pb-2 no-scrollbar">
            {product.images.map((img, i) => (
              <div
                key={i}
                className="min-w-[80px] aspect-square rounded-2xl overflow-hidden border-2 border-border-main hover:border-primary cursor-pointer transition-all bg-white p-1"
              >
                <img
                  src={img}
        ...
        <div className="space-y-10">
          <div>
            ...
            <h1 className="text-5xl font-black text-text-main mb-6 leading-[1.1]">
              {product.name}
            </h1>
          ...
          <div className="glass-card p-8 flex items-center gap-6 bg-gradient-to-br from-white to-primary/5 hover:border-primary/20 transition-all group">
            ...
            <button
              ...
              className="bg-white hover:bg-background border border-border-main px-6 py-3 rounded-2xl text-[11px] text-black font-black uppercase tracking-widest transition-all shadow-sm"
            >
              Xem shop
            </button>
          </div>
          ...
          <div className="bg-text-main rounded-[40px] p-10 text-white relative overflow-hidden shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-10 mb-10 border-b border-white/10 pb-10">
              <div>
                <span className="text-[10px] font-black text-white/50 uppercase tracking-[0.2em] block mb-3">
                  Giá hiện tại
                </span>
              ...
              <div className="text-right">
                ...
                <div
                  className={`text-3xl font-black ${timeLeft < 3600 ? "text-rose-500 animate-pulse" : "text-secondary"}`}
                >
                  {formatTime(timeLeft)}
                </div>
              </div>
